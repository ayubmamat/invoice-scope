...
def upgrade() -> None:
    invoice_source_enum = postgresql.ENUM(
        "upload",
        "email",
        name="invoice_source",
        create_type=False,
    )
    invoice_source_enum.create(op.get_bind(), checkfirst=True)
    ...
    op.create_table(
        ...
        sa.Column("source", invoice_source_enum, nullable=False),
...
def downgrade() -> None:
    ...
    invoice_source_enum.drop(op.get_bind(), checkfirst=True)
